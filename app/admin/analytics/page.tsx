...
export default function AnalyticsPage() {
  ...
  async function checkAdmin() {
    ...
    try {
      ...
      if (userError) {
        ...
        setAuthorized(false);
        setAuthError(
          "Unable to verify your admin session. Please return to Admin and try again."
        );
      ...
      if (!user) {
        setAuthorized(false);
        setAuthError("No active admin session was found.");
        setCheckingAuth(false);
        return;
      }
      ...
      if (adminError) {
        ...
        setAuthorized(false);
        setAuthError(
          "Your session exists, but admin authorization could not be verified."
        );
        setCheckingAuth(false);
        return;
      }
      ...
      console.error(
        "Unexpected analytics authentication error:",
        error
      );
      setAuthorized(false);
      setAuthError(
  ...
  async function loadAnalytics(showRefresh = false) {
    ...
    try {
      ...
      console.error(
        "Unexpected analytics loading error:",
        error
      );
    } finally {
      setLoading(false);
    ...
    for (const event of pageViews) {
      ...
      if (!existing) {
        map.set(event.session_id, {
          ...
          deviceModel:
            event.device_model || "Model unavailable",
          ...
          locationPermission:
            event.location_permission === true,
        ...
        if (
          existing.deviceModel === "Model unavailable" &&
          event.device_model
        ) {
          existing.deviceModel = event.device_model;
        }
        ...
        if (event.location_permission === true) {
  ...
  const sourceRanking = useMemo(() => {
    ...
    visitorSessions.forEach((visitor) => {
      const source = getSource(visitor.referrer);
  ...
  const deviceRanking = useMemo(() => {
    const counts: Record<string, number> = {};
    visitorSessions.forEach((visitor) => {
      counts[visitor.device] = (counts[visitor.device] || 0) + 1;
    });
    return Object.entries(counts)
      .map(([name, count]) => ({ name, count }))
      .sort((a, b) => b.count - a.count);
  ...
  const browserRanking = useMemo(() => {
    const counts: Record<string, number> = {};
    visitorSessions.forEach((visitor) => {
      counts[visitor.browser] = (counts[visitor.browser] || 0) + 1;
    });
    return Object.entries(counts)
      .map(([name, count]) => ({ name, count }))
      .sort((a, b) => b.count - a.count);
  ...
  const activeVisitors = useMemo(() => {
    ...
    return visitorSessions.filter(
      (visitor) =>
        new Date(visitor.lastSeen).getTime() >= fiveMinutesAgo
    ).length;
  }, [visitorSessions]);
  ...
  const selectedSession = selectedVisitor
    ? visitorSessions.find(
        (visitor) => visitor.sessionId === selectedVisitor
      )
    : null;
      ...
      <main className="flex min-h-screen items-center justify-center bg-[#f7f1e4] px-5">
        <div className="w-full max-w-md rounded-3xl bg-white p-8 text-center shadow-lg">
          ...
          <p className="mt-2 text-sm leading-6 text-gray-500">
            Visitor analytics is available only to your administrator account.
          </p>
          {authError && (
      ...
      <section className="mx-auto max-w-[1500px] px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-7 flex flex-col gap-4 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500">
              Showing the last {rangeDays === 1 ? "24 hours" : `${rangeDays} days`}
            </p>
          </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="mt-6 grid grid-cols-3 gap-3 lg:gap-5">
              <div className="rounded-2xl bg-white p-4 shadow-sm">
                <div className="flex items-center gap-2 text-gray-500">
                  <Smartphone size={18} />
                  <span className="text-xs font-bold uppercase">Mobile</span>
                </div>
                <p className="mt-2 text-2xl font-black">{mobileVisitors}</p>
              </div>
              <div className="rounded-2xl bg-white p-4 shadow-sm">
                <div className="flex items-center gap-2 text-gray-500">
                  <Monitor size={18} />
                ...
                <p className="mt-2 text-2xl font-black">{desktopVisitors}</p>
              </div>
              <div className="rounded-2xl bg-white p-4 shadow-sm">
                <div className="flex items-center gap-2 text-gray-500">
                  <Tablet size={18} />
                ...
                <p className="mt-2 text-2xl font-black">{tabletVisitors}</p>
              </div>
            </div>

            <div className="mt-6 grid gap-5 lg:grid-cols-2">
              <RankingList title="Most Visited Pages" items={pageRanking} />
              <RankingList title="Most Viewed Products" items={productRanking} />
              <RankingList title="Traffic Sources" items={sourceRanking} />
              <RankingList title="Browsers" items={browserRanking} />
              <RankingList title="Devices" items={deviceRanking} />
            </div>
            ...
            <section className="mt-7 overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
              <div className="border-b border-gray-100 p-5">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <h2 className="text-xl font-black">Visitor Sessions</h2>
                    <p className="mt-1 text-sm text-gray-500">
                      Click any visitor to inspect their complete browsing journey.
                    </p>
                  </div>
              ...
              {visitorSessions.length === 0 ? (
                <div className="p-12 text-center">
                  ...
                  <p className="mt-1 text-sm text-gray-400">
                    Visitors will appear here as they browse your store.
                  </p>
                </div>
              ) : (
                <div className="divide-y divide-gray-100">
                  {visitorSessions.map((visitor, index) => {
                    const isSelected =
                      selectedVisitor === visitor.sessionId;

                    return (
                      <div key={visitor.sessionId} className="p-5">
                        <button
                          type="button"
                          onClick={() =>
                            setSelectedVisitor(
                              isSelected ? null : visitor.sessionId
                            )
                          }
                            ...
                            <div className="flex min-w-0 items-start gap-4">
                              ...
                              <div className="min-w-0">
                                <p className="truncate text-sm font-black text-gray-900">
                                  Visitor {visitor.sessionId.slice(0, 8)}
                                </p>
                                <p className="mt-1 text-xs text-gray-400">
                                  {visitor.events.length} page
                                  {visitor.events.length === 1 ? "" : "s"} visited
                                </p>
                        ...
                        {isSelected && (
                          <div className="mt-5 rounded-2xl bg-gray-50 p-4">
                            <div className="mb-4 flex flex-wrap items-center gap-2">
                              ...
                              <span className="rounded-full bg-white px-2.5 py-1 text-xs text-gray-500">
                                GPS permission: {visitor.locationPermission
                                  ? "Granted"
                                  : "Not granted"}
                              </span>
                              ...
                              {visitor.events[0]?.screen_width &&
                                visitor.events[0]?.screen_height && (
                                  <span className="rounded-full bg-white px-2.5 py-1 text-xs text-gray-500">
                                    {visitor.events[0].screen_width} × {visitor.events[0].screen_height}
                                  </span>
                                )}
                            </div>

                            <div className="space-y-3">
                              {visitor.events.map((event, eventIndex) => (
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              ...
              <div className="inline-flex items-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-medium text-gray-500 shadow-sm">
                ...
                Device and consent-based location data shown when available
              </div>
            </div>
          </>
        )}
